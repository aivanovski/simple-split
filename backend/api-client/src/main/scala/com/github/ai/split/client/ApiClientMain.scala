  ...
  private def processArguments(arguments: String) = defer {
    ...
    val response = arguments match {
      ...
      case s"update-group $groupUid $other" =>
        val map = parseOptionsIntoMap(other, "--title", "--password", "--description").run
        // Validate that at least one option is provided
        if (map.isEmpty) {
          ZIO.fail(InvalidCliArgumentException("At least one of --title, --password, or --description must be provided")).run
        } else {
          api.updateGroup(
            groupUid = groupUid,
            title = map.get("--title"),
